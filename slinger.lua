--[[
    SLINGERHUB V6.7 - PREVIEW CLEAN VERSION
    - BYPASS TOTAL: No Whitelist / No Self-Destruct
    - UI: WindUI (Blue Theme)
    - FEATURES: Blatant Rolling, Shop (Sell/Weather), Teleport Pro
]]
...
local Window = WindUI:CreateWindow({
    Title = "SlingerHub V6.7 | ANTI-SELF DESTRUCT",
    Icon = "zap",
    Author = "SlingerDev",
    Folder = "SlingerHub_V67",
...
ShopSection:Toggle({ Title = "Auto Sell (60s)", Callback = function(v) 
    state.AutoSell = v 
    task.spawn(function() while state.AutoSell do if net then net["RF/SellAllItems"]:InvokeServer() end task.wait(60) end end)
end })
ShopSection:Toggle({ Title = "Auto Buy Weather (Rain)", Callback = function(v) 
    state.AutoBuyWeather = v 
    task.spawn(function() while state.AutoBuyWeather do if net then net["RF/PurchaseWeather"]:InvokeServer("Rain", 1) end task.wait(30) end end)
end })

-- TELEPORT SECTION
local PosSection = TeleportTab:Section({ Title = "Position Manager" })
PosSection:Button({ Title = "Save Position", Callback = function() state.SavedPos = LocalPlayer.Character.HumanoidRootPart.CFrame WindUI:Notify({Title="Success", Content="Spot Saved!"}) end })
...
WindUI:Notify({Title = "SlingerHub V6.7", Content = "Script Loaded! No Whitelist Needed."})
